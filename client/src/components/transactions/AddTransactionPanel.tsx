    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative h-full w-full max-w-md bg-white dark:bg-zinc-900 border-l border-slate-200 dark:border-zinc-800 flex flex-col">
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-200 dark:border-zinc-800 shrink-0">
          <h2 className="text-sm font-semibold text-slate-900 dark:text-zinc-100">Add transaction</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col overflow-hidden">
          <div className="flex-1 overflow-y-auto px-6 py-6 space-y-5">
            <div>
              <label className="text-xs font-medium text-slate-500 dark:text-zinc-400 mb-2 block">Type</label>
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className={`text-sm font-medium py-2.5 rounded-lg border transition-colors ${
                    type === 'EXPENSE'
                      ...
                      : 'border-slate-200 dark:border-zinc-800 text-slate-500 dark:text-zinc-400 hover:bg-slate-100 dark:hover:bg-zinc-800/60'
                  }`}
                >
                  Expense
                </button>
                <button
                  ...
                  className={`text-sm font-medium py-2.5 rounded-lg border transition-colors ${
                    type === 'INCOME'
                      ? 'bg-emerald-500/10 border-emerald-500/30 text-green-700 dark:text-emerald-400'
                      : 'border-slate-200 dark:border-zinc-800 text-slate-500 dark:text-zinc-400 hover:bg-slate-100 dark:hover:bg-zinc-800/60'
                  }`}
                >
                  Income
            ...
            <div>
              <label htmlFor="category" className="text-xs font-medium text-slate-500 dark:text-zinc-400 mb-2 block">
                Category
              </label>
              <select
                id="category"
            ...
            <div>
              <label htmlFor="date" className="text-xs font-medium text-slate-500 dark:text-zinc-400 mb-2 block">
                Date
              </label>
              <input
                id="date"
