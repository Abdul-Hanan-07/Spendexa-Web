    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative h-full w-full max-w-md bg-zinc-900 border-l border-zinc-800 flex flex-col">
        <div className="flex items-center justify-between px-6 py-5 border-b border-zinc-800 shrink-0">
          <h2 className="text-sm font-semibold text-zinc-100">Add loan</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col overflow-hidden">
          <div className="flex-1 overflow-y-auto px-6 py-6 space-y-5">
            <div>
              <label htmlFor="principal" className="text-xs font-medium text-zinc-400 mb-2 block">
                Principal
              </label>
              <input
                id="principal"
            ...
            <div>
              <label htmlFor="interestRate" className="text-xs font-medium text-zinc-400 mb-2 block">
                Interest rate (%)
              </label>
              <input
                id="interestRate"
            ...
            <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-3.5 text-center">
              <p className="text-xs text-zinc-400">Total to repay</p>
              <p className="text-lg font-bold text-amber-400 mt-0.5">
                {hasValidPreview ? formatCurrency(totalToRepay, currency) : '—'}
              </p>
              <p className="text-[11px] text-zinc-500 mt-0.5">Principal + principal &times; rate / 100</p>
            </div>
