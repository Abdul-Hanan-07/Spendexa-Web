...
export function Sidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-64 bg-zinc-900 border-r border-zinc-800 flex flex-col transform transition-transform duration-200 ease-in-out md:translate-x-0 md:static md:z-auto ${
      ...
      >
        <div className="flex items-center justify-between px-5 h-16 border-b border-zinc-800 shrink-0">
          ...
          <button onClick={onClose} className="md:hidden text-zinc-400 hover:text-zinc-200">
            <X size={20} />
          </button>
        </div>
